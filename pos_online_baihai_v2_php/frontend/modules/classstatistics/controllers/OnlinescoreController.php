...
class OnlinescoreController extends ClassesBaseController
{
    ...
    public function actionIndex($classId,$selClassId=0){
        $proFirstime = microtime();
        $this->getClassModel($classId);
        $schoolExamId = app()->request->get('examId');
        $selClassId = app()->request->get('selClassId','');
        /**
         * 表格
         */
        //分数线
        ...
        \Yii::info('上线分数 '.(microtime()-$proFirstime),'service');
     ...
     * 上线数图表数据
     */
    public function onlineNum($examId,$classId){
