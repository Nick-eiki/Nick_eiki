...
class DefaultController extends ClassesBaseController
{
    ...
    public function actionIndex($classId)
    {
        ...
        $classData = SeClass::find()->where(['classId'=>$classId,'isDelete'=>0])->one();
        ...
        \Yii::info('班级统计 '.(microtime()-$proFirstime),'service');
     ...
     * 统计概览
     */
    public function actionOverview($classId)
    {
        $proFirstime = microtime();
        ...
        \Yii::info('统计概览 '.(microtime()-$proFirstime),'service');
        if (app()->request->isAjax) {
            if (!empty($subjectId)) {
     ...
     *班级对比
     */
    public function actionClassesContrast($classId)
    {
        $proFirstime = microtime();
        $this->getClassModel($classId);
        $schoolExamId = app()->request->get('examId');
        ...
        \Yii::info('班级对比 '.(microtime()-$proFirstime),'service');
        if (app()->request->isAjax) {
            if ($num == 1) {
