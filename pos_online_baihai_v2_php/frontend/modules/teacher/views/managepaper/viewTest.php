...
';?>
    ...
    <div class="work_detais_cent">
        <h4><?php echo $data->testName ?></h4>
        <ul class="detais_list">
            <li>
                <span>地区：<i><?php echo AreaHelper::getAreaName($data->provience) ?> <?php echo AreaHelper::getAreaName($data->city) ?> <?php echo AreaHelper::getAreaName($data->country) ?></i></span><span>年级：<i><?php echo GradeModel::model()->getGradeName($data->gradeId) ?></i></span><span>科目：<i><?php echo SubjectModel::model()->getSubjectName($data->subjectId) ?></i></span><span>版本：<i><?php echo EditionModel::model()->getEditionName($data->version) ?></i></span>
            </li>
            ...
            <li><span>学生成绩：<i><?php foreach($studentAnswer->scoreList as $v){
                            echo $v->studentName.":".$v->testScore."分"."&nbsp";
                        }?></i></span></li>
        </ul>
        <div class="details_b" id="details_b">
            <div class="species clearfix">
                ...
                <div class="species_Right">
                    <ul class="minute clearfix ">
                        <?php if (!empty($data->imageUrls)) {
                            foreach (ImagePathHelper::getPicUrlArray($data->imageUrls) as $k => $v) {
                                if ($k == 0) {
                                    ?>
                                    <li>
                <span>
                    <img alt="" src="<?php echo $v ?>">
                    <em><a href="<?php echo url('teacher/managepaper/paper-preview', array('testID' => app()->request->getParam('testId'))) ?>">试卷预览</a></em>
                </span>
                ...
                <div class="answer">
                    <div class="answer_list clearfix">
                        <span class="answer_left fl">
                            ...
                            <em class="pagination">共<?php echo count($v->testCheckInfoS) ?>
                                页(<?php echo $v->isCheck ? "批改完成" : "未批改" ?>)</em>
                         </span>
                    ...
                    <div class="test_paper test_paper_list" style="display:block;">
                        ...
                        <?php if (!$v->isCheck) { ?>
                            <p class="btn"><a
                                    href="<?php echo url('teacher/managepaper/correct-paper', array('testAnswerID' => $v->testAnswerID)) ?>">我要判卷</a>
                            </p>
                        <?php } else { ?>
                            <p class="btn"><a
                                    href="<?php echo url('teacher/managepaper/view-correct', array('testAnswerID' => $v->testAnswerID)) ?>">查看批改</a>
                            </p>
                        <?php } ?>
                    </div>
        ...
        <hr>
<!--        --><?php //if($answerResult->notCheckAnswerNum>1||!($answerResult->isHaveCrossCheck)){?>
<!--        <div class="btn_work_ails">-->
<!--            <input type="checkbox" class="chck" id="check">-->
<!--            <button type="button" class="btn btn_color" id="pjjr_js">学生互相判卷</button>-->
<!--        </div>-->
<!--        --><?php //}?>
            <?php
...
<div id="sends" class=" popBox mutual_pjjr_pop hide" title="同学互相判卷">
    <div class="impBox">
        ...
        <div class="mutual_pjjr_list">的所有学生互换试卷并进行批改吗?</div>
...
<!--                    <div>--><?php //echo $data->className?><!--</div>-->
            ...
            <li>
                ...
                <div class="formR">
                    <div id="point_chapt" class="treeParent"><i class="ico"></i>
            ...
            <li>
                ...
                <div class="formR">
                    <textarea class="learningPlan">
                        <?php echo $evaluateResult->summary?>
                    </textarea>
                </div>
            </li>
        </ul>
