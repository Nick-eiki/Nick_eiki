...
<div class="testPaperView pr">
    <div class="paperArea">
        <?php if(!empty($item)){foreach($item->list as $tnum=>$val){ ?>
            <div class="paper">
                <h5>题目<?php echo $tnum+1 ?>:</h5>
                <h6><?php if(!empty($val->year)){ $str = "【".$val->year."年】 ";}else{ $str = '';}echo $str.$val->provenanceName." ".$val->questiontypename;?><a href="<?php echo url('/teacher/testpaper/topicedit');?>?topic=<?php echo $val->id ?>" class="a_button bg_green fr">题目修改</a></h6>
...
<script>
    ...
    $(function(){
        ...
        $('.Determine').live('click',function(){
            var $this = $(this);
            var url= "<?php echo url('/teacher/testpaper/modifycomplexity');?>";
