                ...
                if(code){
                    ...
                    $.post($form_id.attr('action'), $form_id.serialize(),function(data){
                        if(data.success){
                            ...
                            $.get("<?php echo Url::to(['/teacher/managetask/one-work-content'])?>", {hmwid: hmwid}, function (result) {
                    ...
                    <div class="formR formR_text">
                        <?php foreach($homeworkRelList as $assignClass){?>
                            <dl class="myclass_table myclass_tablebox">
                                <dt class="curnone">
                                  <h5 class="ification" title="<?=WebDataCache::getClassesName($assignClass->classID)?>"><?=WebDataCache::getClassesName($assignClass->classID)?></h5>
                                </dt>
                                <dd>
                                    <span>交作业截止时间:</span>
                ...
                <div class="row clearfix">
                    ...
                    <div class="formR formR_text">
                        ...
                        <dl class="myclass_table unmyclass_table">
                            <dt data-id="<?= $val?>">
                                <input type="checkbox"/>
                                <span title="<?= Html::encode(WebDataCache::getClassesName($val))?>"><?= Html::encode(WebDataCache::getClassesName($val))?></span>
                            </dt>
