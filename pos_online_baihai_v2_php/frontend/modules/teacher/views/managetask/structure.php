...
$this->registerJsFile($backend_asset . '/js/ztree/jquery.ztree.all-3.5.min.js', ['position'=>View::POS_END]);
?>
<div class="currentRight grid_16 push_2 make_testpaper">
    ...
    <p class="tc bottomBtnBar">
        <button type="button" id="nextstep" class="nextStepBtn">下一步</button>
    </p>
</div>
    ...
    $("#nextstep").click(
        function () {
            ...
            $.post($('#makePaper').attr('action'), $.param(formarray), function (result) {
                if (result.success) {
                    window.location.href = "<?php  echo url("/teacher/managetask/subject")  ?>?homeworkId=" + result.data;
...
var testPaperCont =<?php echo json_encode($this->toZtreeData($treejson)); ?>;

var num=["一","二","三","四","五","六","七","八","九","十","十一","十二","十三","十四","十五"];
var setting2 = {
    check:{
        enable:true,
        chkboxType:{"Y" : "", "N" : ""}
...
//试卷头部
    ...
    for(var i=0; i<headIndex; i++){
        ...
        if(id=="secret_sign"||id=="main_title"||id=="sub_title"||id=="info"||id=="student_input"){
            ...
            if(testPaperCont.paperHead[i].pId=="testPaperHead"){$('#testPaperHead').append(headPart)}
        }
...
//试卷插入项目
    ...
    for(var i=0; i<subject; i++){
        ...
        if(testPaperCont.paperBody[i].pId=="testPaperBody"){$('#testPaperBody').append(paperPart)}
        if(testPaperCont.paperBody[i].pId=="win_paper_typeone"){$('#win_paper_typeone').append(subPart)}
        if(testPaperCont.paperBody[i].pId=="win_paper_typetwo"){$('#win_paper_typetwo').append(subPart)}
