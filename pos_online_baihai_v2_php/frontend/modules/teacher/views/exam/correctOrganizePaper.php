...
<!--        <h5>--><?php //echo $testResult->studentName."的".$testResult->name?><!--</h5>-->
        ...
        <div class="correctPaperSlide">
            <div class="testPaperWrap mc">
                <ul class="testPaperSlideList slid">
                    <?php foreach($testResult->picList as $v){?>
                        <li><img src="<?php echo publicResources().$v->picUrl?>" width="830" height="508"  alt=""/>
            ...
            <br>
            <div class="tc bottomBtnBar"><button type="button" class="w120 bg_green correctEndBtn">批改完成</button></div>
...
//  批改完成
        $( ".correctEndBtn" ).click(function(){
            ...
            $.post("<?php echo url('teacher/managepaper/finish-correct')?>",{"testScore":totalScore,"testAnswerID":testAnswerID},function(result){
                if(result.code==1){
...
//        保存本页批改
        $(".finish").click(function(){
            ...
            var testAnswerID="<?php echo app()->request->getParam('testAnswerID')?>";
            $.post("<?php echo url('teacher/managepaper/hold-organize-correct')?>",{"checkInfoJson":JSON.stringify(checkInfoList),"picID":picID,"answerID":answerID,"testAnswerID":"<?php echo app()->request->getParam('testAnswerID')?>"},function(result){
                if(result.code){
                    popBox.alertBox(result.message);
                }
                else{
