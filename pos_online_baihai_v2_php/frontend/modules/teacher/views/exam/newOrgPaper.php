    ...
    function savePage() {
        ...
        tc = setTimeout(function () {
            ...
            $('li.current div.tips').each(function(index,el){
                ...
                var checkInfo={"style":style,"comments":scoreTxt,"scoreVal":scoreVal};
                checkInfoArray.push(checkInfo);
            });
            var checkInfoList={"checkInfoList":checkInfoArray};
            var picID=$("li.current").find(".picID").val();
            var answerID=$("li.current").find(".answerID").val();

            var testAnswerID="<?php echo app()->request->getParam('testAnswerID')?>";
            $.post("<?php echo url('teacher/exam/hold-organize-correct')?>",{"checkInfoJson":JSON.stringify(checkInfoList),"picID":picID,"answerID":answerID,"testAnswerID":"<?php echo app()->request->getParam('testAnswerID')?>"},function(result){
                if(result.code){
                    popBox.success(result.message);
                }
                else{
                    popBox.errorBox(result.message);
                }
...
<div id="dati" class="popBox correctEndBox" style="display: none" title="批改完成">
    ...
    <div class="popCont" style="padding-left:100px">
        ...
        <br>
        <p>得分 <span id="score" class="text w50"></span> 分
        </p>
        <!--            &nbsp;&nbsp;&nbsp;&nbsp;满分 100 分-->
        <br>
                                ...
                                if (!empty($checkInfoList->checkInfoList)) {
                                    foreach ($checkInfoList->checkInfoList as $value) { ?>
                                        <div class="tips" style="<?php echo $value->style ?>">
                                            ...
                                            <br>
                                            <?php if(isset($value->scoreVal)){?>
                                                <strong class="scoreVal"><?=$value->scoreVal?></strong>
                                                分
                                            <?php }?>
                                            <span class="removeBtn hide" style="display: none;">×</span>
                                        </div>
                                    <?php }
                                } ?>
                            </li>
