...
//  批改完成
        $( ".correctEndBtn" ).click(function(){
            var testAnswerID="<?php echo app()->request->getQueryParam('otherTestAnswerID')?>";
            var totalScore=0;
...
//        保存本页批改
        $(".finish").click(function(){
            ...
            $.post("<?php echo url('student/managepaper/hold-org-correct')?>",{"checkInfoJson":JSON.stringify(checkInfoList),"picID":picID,"answerID":answerID,"testAnswerID":"<?php echo app()->request->getQueryParam('otherTestAnswerID')?>"},function(result){
                if(result.code){
                    popBox.alertBox(result.message);
                }
                else{
