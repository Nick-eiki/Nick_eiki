 ...
 * Time: 下午6:38
 */
/* @var $this yii\web\View */  $this->title="判卷";
?>
        ...
        <div class="correctPaperSlide">
            <div class="testPaperWrap mc">
                <ul class="testPaperSlideList">
                    <?php foreach ($answerResult->homeworkAnswerImages as $v) { ?>
                        <li><img src="<?php echo publicResources() . $v->imageUrl ?>" width="830" height="508"
                                 alt=""/></li>
                        <input type="hidden" class="tID" value="<?php echo $v->tID ?>"/>
...
//                var score=$(el).find(".scoreVal").html();
            ...
            $.post("<?php  echo url('student/managetask/hold-correct')?>", {checkInfoJson: JSON.stringify(checkInfoList), homeworkAnswerID: "<?php echo app()->request->getQueryParam('homeworkAnswerID')?>", "tID": tID}, function (result) {
...
//        批改完成
        $(".correctEndBtn").click(function () {
            $.post("<?php echo url('student/managetask/finish-correct')?>", {"homeworkAnswerID":"<?php echo app()->request->getQueryParam('homeworkAnswerID')?>"}, function (result) {
                if (result.code == 1) {
                    popBox.alertBox(result.message);
                    location.href = "<?php echo url('student/managetask/upload-details',array('homeworkID'=>app()->request->getQueryParam('homeworkID')))?>";
                } else {
                    popBox.alertBox(result.message);
                }
            })
