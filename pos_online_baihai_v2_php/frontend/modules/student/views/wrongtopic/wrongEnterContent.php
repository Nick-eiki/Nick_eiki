 ...
 * Time: 15:36
 */
use frontend\widgets\ueditor\MiniNoImgUEditor;
use yii\helpers\Url;
...
$(function(){
    ...
    $('.viewBtn').bind('click',function(){
        ...
        $.post(url,{'id':id,'answerOptionJson':JSON.stringify(obj.sel),'answerContent':obj.answer,'analytical':obj.analy,'childQuesJson':JSON.stringify(obj.subItem),'saveType':0},function(msg){
            if(msg.success==true){
                ...
                $.post("<?php echo Url::to(['/student/wrongtopic/view-topic'])?>",{'id':id},function(e){
                    preview.viewBox(e);
                })
            }
            //var html=preview.getViewHtml(preview.showType);
    ...
    $('.saveBtn').click(function(){
        ...
        var url='<?php echo url("/student/wrongtopic/save-ques-content")?>';
        var id = $("#Id").val();
        var str = '';
        ...
        $.post(url,{'id':id,'answerOptionJson':JSON.stringify(obj.sel),'answerContent':obj.answer,'analytical':obj.analy,'childQuesJson':JSON.stringify(obj.subItem),'saveType':1},function(msg){
            if(msg.success==true){
                location.href="<?php echo url('/student/wrongtopic/topic-finish')?>?status=1";
            }
            else{
                location.href="<?php echo url('/student/wrongtopic/topic-finish')?>?status=0";
            }
        })
    });
    //保存草稿箱
                        ...
                        <div class="testPaperView">
                            <div class="paperArea">
                                <div class="paper"><!--选择题-->
                                    ...
                                    <p class="q_content"><?php echo $info->content?></p>
                                </div>
                            </div>
                        </div>
                    </div>
