...
<div class="aside col260 alpha">
    <div id="sel_classes" class="currency_hg sel_classes">
        <div class="pd15">
            <h5><?=WebDataCache::getDictionaryName($department).SubjectModel::model()->getSubjectName($subjectId)?></h5>
...
<div class="asideItem">
    <div class="border  currency_hg1">
        <div id="sel_course" class="sUI_select" style="border-bottom: 1px solid #f5f5f5">
            <em class="sUI_select_t"><?=EditionModel::model()->getEditionName($version)?></em>
        ...
        <?php if(!empty($chapterTomeResult)){?>
            <div id="sel_grade" class="sUI_select">
                <em class="sUI_select_t"><?=ChapterInfoModel::getNamebyId($chapterId)?></em>
                <ul class="sUI_selectList pop">
                    <?php foreach($chapterTomeResult as $v){ ?>
                        <li><a href="<?=url::to(array_merge([''],$searchArr,['version'=>$version,'chapterId'=>$v->cid]))?>"><?=$v->chaptername?></a></li>
                    <?php }?>
                </ul>
                <i class="sUI_select_open_btn"></i>
            </div>
