        ...
        <?php
            if($level==0){
                foreach($dataList as $data){
                    $schoolId=WebDataCache::getSchoolId($data->creator);
                    $schoolName=WebDataCache::getSchoolName($schoolId);
                    $subjectName=SubjectModel::model()->getSubjectName($data->subjectId);
                    $departmentName=WebDataCache::getDictionaryName($data->department);
                    $gradeName=\frontend\models\dicmodels\GradeModel::model()->getGradeName($data->gradeId);
                    $editionName=EditionModel::model()->getEditionName($data->version);
            ...
            <li class="<?php if($data->isNewHomework()){echo 'news';} ?>">
                ...
                <h5><a href="<?=Url::to(['/teacher/managetask/pushed-library-details','homeworkID'=>$data->id])?>"><?php echo '<em>'.ChapterInfoModel::getNamebyId($data->chapterId).'('.$departmentName.$subjectName.$editionName.$gradeName.')</em>'.'&nbsp&nbsp'. AreaHelper::getAreaName($data->provience);?>·<?php echo AreaHelper::getAreaName($data->city);?>·<?php echo AreaHelper::getAreaName($data->country);?>·<?=$schoolName?></a></h5>
                <h6 class="gray">原标题:<?php echo $data->name;?></h6>
                <p><img class="owner_img" src="<?=WebDataCache::getFaceIcon($data->creator)?>"><?php echo \frontend\components\WebDataCache::getTrueName($data->creator);?> <?php if (!empty($data->uploadTime)) {
...
<!--                <p><img class="owner_img" src="../dev/images/head_70.png">--><?php //echo WebDataCache::getTrueName($data->creator);?><!-- --><?php //if (!empty($data->uploadTime)) {
//                        echo date('Y-m-d', DateTimeHelper::timestampDiv1000($data->uploadTime));
//                    } ?><!-- 贡献到平台</p>-->
                <!--<p>难度:<?php /*echo $data->difficulty;*/?></p>-->
