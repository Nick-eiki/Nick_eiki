    ...
    <li class="test_ques_list">
        <div class="cls_lf_list">
            ...
            <div class="sUI_pannel sUI_pannel_min">
                <h5><a href="<?=Url::to(array_merge(['video/list'],['paperId'=>$val->paperId]))?>" title="<?= $val->paperName ?>"><?= StringHelper::cutStr($val->paperName,33);?></a></h5>
            </div>
            <div class="sUI_pannel in_troduces">
                <span>
                    <?php if(!empty($val->department)){echo $val->department=='20202'?'中考真题'."&nbsp;&nbsp;|&nbsp;":'高考真题'."&nbsp;&nbsp;|&nbsp;";}?>
                    <?php if(!empty($val->year)){echo $val->year.'年';}?>
                </span>
                <span>
                    <?php if(!empty($val->allquestionCount)){echo '总题数:'.$val->allquestionCount;}?>
                </span>
            </div>
        </div>
    </li>
