...
class pos_MessageSentService extends BaseService
{
    ...
    {
        //$soapResult = $this->_soapClient->readerQuerySentMessageInfo(array("receiverUserID" => $receiverUserID,"mainType" => $mainType,  "messageType" => $messageType, "currPage" => $currPage, "pageSize" => $pageSize));
     ...
     * "currPage":"1","listSize":1,"totalPages":"1"},"resCode":"000000","resMsg":"查询成功"
     */
    public function senderQuerySentMessageInfo($messageID, $fromUserid, $messageType, $currPage, $pageSize)
    {
        $soapResult = $this->_soapClient->senderQuerySentMessageInfo(array("messageID" => $messageID, "fromUserid" => $fromUserid, "messageType" => $messageType, "currPage" => $currPage, "pageSize" => $pageSize));
        return $this->soapResultToJsonResult($soapResult);
    }
     ...
     * }
     */
    public function senderMessageDelet($messageID)
    {
        $soapResult = $this->_soapClient->senderMessageDelet(array("messageID" => $messageID));
        return $this->soapResultToJsonResult($soapResult);
    }
     ...
     * 未读的条数
     * unReadNum 方法名
     * @param $receiverUserID           消息标题
     * {"data":{"unReadCount":"5"},"resCode":"000000","resMsg":"成功"}
     */
    public function unReadNum($receiverUserID)
    {
        if (empty($receiverUserID)) {
            return 0;
        }
        $soapResult = $this->_soapClient->unReadNum(array("receiverUserID" => $receiverUserID));
