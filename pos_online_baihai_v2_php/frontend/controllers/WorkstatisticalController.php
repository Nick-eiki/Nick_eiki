...
class WorkstatisticalController extends BaseAuthController
{
    ...
    public function actionWorkStatisticalAll()
    {
        ...
        $this->getClassModel($result->classID);
        ...
        \Yii::info('总体分析 ' . (microtime() - $proFirstime), 'service');
    ...
    public function actionWorkStatisticalTopic($relId)
    {
        ...
        foreach ($questionInfoList as $questionModel) {
            $showType = $questionModel->isMajorQuestionCache();
            $arrQuestionId = $homeworkTeacherOne->getQuestionNo($questionModel->id);
        ...
        foreach ($homeworkQuestions as $complexity => $complexityCount) {
            ...
            if (!array_key_exists($complexityCount['complexity'], $arr)) {
                ...
                $complexityName = DegreeModel::model()->getDegreeName($complexity);
        ...
        \Yii::info('题目分析 ' . (microtime() - $proFirstime), 'service');
    ...
    public function actionWorkStatisticalStudent($relId)
    {
        ...
        foreach ($homeworkAnswerID as $v) {
            ...
            if ($allCount == 0) {
                $orderStudent[] = ['studentId' => $v->studentID, 'per' => 0];
            } else {
                $orderStudent[] = ['studentId' => $v->studentID, 'per' => $rightCount / $allCount];
            }
        }

        if ($homeworkAnswerID) {
            //查询学生的作业
            $questionArray = SeHomeworkAnswerQuestionAll::find()->select('questionID')->where(['relId' => $relId, 'studentID' => $homeworkAnswerID[0]->studentID])->asArray()->all();
        }
        ...
        foreach ($questionArray as $v) {
            $questionIdResult[$v['questionID']] = $homeworkResult->getQuestionNo($v['questionID']);
        }
        asort($questionIdResult);

        \Yii::info('学生分析 ' . (microtime() - $proFirstime), 'service');
        return $this->render("work_statistical_student", ['homeworkAnswerID' => $homeworkAnswerID, 'relId' => $relId, 'orderStudent' => $orderStudent, 'questionIdResult' => $questionIdResult, 'homeworkResult' => $homeworkResult, 'homeWorkTeacher' => $homeWorkTeacher]);
