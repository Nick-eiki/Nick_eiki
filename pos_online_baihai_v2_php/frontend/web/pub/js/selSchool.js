...
function getStateHtml(fn){
	var html = '';
	$.ajax({
		url:urlHttp+'/ajax/get-json-province-list',
		type:'GET',
		dataType:"json",
		data:{},
...
function getCityHtml(id){
	var html = '';
	$.ajax({
		url:urlHttp+'/ajax/get-json-area',
		type:'GET',
		dataType:"json",
		data:{'id':id},
		success: function(msg){
...
function getSchool(county,departmentEl){
	var html = '';
	$.ajax({
		url:urlHttp+'/register/new-search-school-info',
		type:'GET',
		dataType:'html',
...
function searchSchool(searchWord,departmentEl){
	var html = '';
	$.ajax({
		url:urlHttp+'/register/search-school-info',
		type:'GET',
		dataType:'html',
...
function createSchool(departmentEl){
	var html = '';
		html+= '';
		html+= '<div id="addSchool" action="/register/add-school">';
		...
		html+= '<div class="popBox mySchoolBox" id="' + id + '">';
			html+= '<div id="updateSchool">';
				...
				html+= '<div class="popCont">';
					...
					//省市区结束
					//学校列表部分开始
					html+= '<ul id="schoolListInfo" class="resultList schoolList clearfix"></ul>';
					//学校列表部分结束
					//新建学校部分开始
	...
	$('#'+id+' .okBtn').click(function(){
		if($(this).hasClass('createNewBtn')){
			if($('#addSchool').validationEngine('validate')) {
				...
				$.ajax({
					url:urlHttp+'/register/add-school',
					type:'POST',
					data:pro,
					dataType:'json',
			...
			$(targetName).val(schoolName).blur();
			$(targetInput).val(schoolId);
			$('.work_cls_list input').val('');
			$("#"+id).dialog('close');
			$(targetName).blur();
