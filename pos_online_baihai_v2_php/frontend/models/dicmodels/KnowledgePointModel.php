...
class KnowledgePointModel
{
     ...
     * @param $subjectID    科目
     * @param $departmentID 学籍
     * @return array
     */
    public static function searchAllKnowledgePoint($subjectID, $departmentID)
    {
        $resultArr = [];
        if (empty($subjectID) && empty($departmentID)) {
            return [];
        }
        $list = SrKnowledgepoint::find()->where(['subject' => $subjectID, 'grade' => $departmentID])->select('kid,pid,kpointname,subject,grade')->limit(10000)->all();
        foreach ($list as $val) {

            $subjectName = SubjectModel::model()->getSubjectName($val->subject);

            $gradeName = SchoolLevelModel::model()->getSchoolLevelhName($val->grade);
            $array['kid'] = $val->kid;
            $array['pid'] = $val->pid;
            $array['kpointname'] = $val->kpointname;
            $array['subjectName'] = $subjectName;
            $array['gradeName'] = $gradeName;
            $arr[] = $array;
        }
        $callback =
            function ($item) {
                ...
                $k->pId = $item['pid'];
                $k->name = $item['kpointname'];
                $k->subject = $item['subjectName'];
                $k->schoolLevel = $item['gradeName'];
                return $k;
            };
        foreach ($arr as $item) {
            $resultArr[] = $callback($item);
    ...
    public function pub($v)
    {
        ...
        if ($result === false) {
            $arr = SrKnowledgepoint::find()->where(['kid' => $v])->one();
            if (empty($arr)) {
                return null;
            }
            $subjectName = SeDateDictionary::find()->where(['secondCode' => $arr->subject])->select('secondCodeValue')->one();
            $gradeName = SeDateDictionary::find()->where(['secondCode' => $arr->grade])->select('secondCodeValue')->one();
            $result['id'] = $arr->kid;
            $result['pId'] = $arr->pid;
