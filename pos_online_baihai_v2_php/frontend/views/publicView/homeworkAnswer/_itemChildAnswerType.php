...
<div class="middleTitle">
    ...
    <?php if ($item->getQuestionShowType() == 1 || $item->getQuestionShowType() == 2) { ?>
        ...
        <p><span>小题<?php echo $homeworkResult->getQuestionNo($item->id) ?></span>
    ...
    <?php if ($item->getQuestionShowType() == 3 || $item->getQuestionShowType() == 4   || $item->getQuestionShowType()==5 ) { ?>
        <p><span>小题<?php echo $homeworkResult->getQuestionNo($item->id) ?></span>: <?php echo $item->content ?></p>
    <?php } ?>

    <?php if ($item->getQuestionShowType() == 9) { ?>
        <p><?php echo $homeworkResult->getQuestionNo($item->id).$item->content ?></p>
        <div class="checkArea">
            <?php
            $op_list = array(
