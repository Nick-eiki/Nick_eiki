        ...
        $('.q_add').die().live('click', function () {
            ...
            if (creatorid == userid) {
                ...
                $.post('<?php echo url('answer/same-question');?>', {aqid: aqid}, function (data) {
                    if (data.success) {
	                    $('#come_from'+aqid ).children('ul').append('<li><div class="picture_listl" creatorID="<?=user()->id; ?>"><img data-type="header" onerror="userDefImg(this);"  width="30px" height="30px" src="<?php echo publicResources() . WebDataCache::getFaceIcon(user()->id);?>" alt="" title="<?php echo WebDataCache::getTrueName(user()->id);?>"></div></li>');
