		...
		<li class="clearfix">
			<em class="answer_listL" creatorID="<?=$msgValue->creatorID?>" source="<?=$msgValue->sourceChannel?>">
				<img data-type="header" onerror="userDefImg(this);" width="50" height="50"
				     src="<?php echo $msgValue->sourceChannel==0?publicResources() . WebDataCache::getFaceIcon($msgValue->creatorID):'http://f.kehai.com/file/userFace/'.$msgValue->creatorID.'.r'?>">
                <?php if($msgValue->sourceChannel==1){?>
                    <div class="identity"><em class="example">榜</em></div>
                <?php }else{ if(WebDataCache::getUserType($msgValue->creatorID)==0){?>
				...
				<div class="answer_a clearfix">
					...
					if ($question->creatorID == user()->id) {
						if ($msgValue->isUse == 0 && $isUse == true) {
							?>
							<span class="put put_Js adopt_btn" val="<?php echo $msgValue->resultID;?>" u="<?php echo $msgValue->rel_aqID;?>">
								<a href="javascript:" class="btn_c">采用</a>
							</span>
						<?php }
					} ?>

					<em class="fr"><?php echo date('Y-m-d', DateTimeHelper::timestampDiv1000($msgValue->createTime)); ?></em>
				</div>
				<div class="answer_a clearfix">
					...
					foreach ($resultImg as $k => $resultImgSrc) {
						...
						<a class="fancybox" href="<?php echo $resultImgSrc; ?>"
						   data-fancybox-group="gallery_<?= $msgValue->resultID; ?>"><img width="162" height="122"
						                                                                  src="<?php echo $resultImgSrc; ?>"
						                                                                  alt=""></a>
					<?php } ?>
				</div>
			</div>
