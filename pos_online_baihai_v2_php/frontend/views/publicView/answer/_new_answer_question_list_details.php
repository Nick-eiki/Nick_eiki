...
<li class="QA_li  ">
	<div class="sUI_pannel userInfo <?php if($val->isSolved== 0){ echo ''; }elseif($val->isSolved == 1){ echo 'solve';} ?>">
		<div class="pannel_l head_card" creatorID="<?=$val->creatorID; ?>">
			<img class="icon_card" data-type="header" onerror="userDefImg(this);"  width="70px" height="70px" src="<?php echo ImagePathHelper::imgThumbnail(WebDataCache::getFaceIcon($val->creatorID),70,70);?>" alt="" creatorID="<?=$val->creatorID; ?>" source="0">
			<h5><?php echo $val->creatorName; ?></h5>
			<p> <?php echo SubjectModel::model()->getSubjectName($val->subjectID)."&nbsp" ; echo date("Y年m月d日 H:i", DateTimeHelper::timestampDiv1000($val->createTime)); ?></p>
		</div>
		<div class="pannel_r">
...
<!--		<div class="pannel_r"><span class="gray">举报</span></div>-->
<!--	</div>-->
	<div class="pd15">
		<h4><?php echo Html::encode($val->aqName); ?></h4>
	</div>
	<div class="QA_txt"><?php echo StringHelper::htmlPurifier($val->aqDetail); ?></div>
	<?php
	//分离图片
	$img = ImagePathHelper::getPicUrlArray($val->imgUri);
	foreach($img as $k=>$imgSrc) {
	?>
		<a class="fancybox" href="<?php  echo $imgSrc; ?>" data-fancybox-group="gallery_<?= $val->aqID; ?>">
			<img class="lazy" width="120" height="90" data-original="<?=ImagePathHelper::imgThumbnail($imgSrc,120,90) ?>" alt="">
		</a>
	<?php  } ?>
