		...
		<div class="monthbox">
			<ul class="worklist">
				...
				foreach ($newHomeworkGVal as $key => $val) {
					...
					//查询批改数
					$isCorrections = $val->isCheckedStudentCountCache();
					if (!empty($homeworkInfo)) {
						?>
						<!-- 年月份开始 -->

						<li class="clearfix <?php if($isTop){ echo 'work_notop'; $isTop=false;} ?>" relId="<?=$val->id?>">
							...
							<div class="work_left">
								<div class="work_top clearfix">
									...
									<div class="work_info">
                                        <p class="work_title">
	                                        <a title=" <?php echo Html::encode($homeworkInfo->name) ;?>" href="<?php if($homeworkInfo->getType == 0){  echo Url::to(['up-details','homeworkId'=>$homeworkInfo->id,'classId'=>$classId]); }else{ echo Url::to(['organize-details','homeworkId'=>$homeworkInfo->id,'classId'=>$classId]); }?>">
		                                        <?php echo Html::encode($homeworkInfo->name);?>
	                                        </a> </p>
										<p> 截止：<?php echo date('Y年m月d日',DateTimeHelper::timestampDiv1000($val->deadlineTime)); ?></p>
									</div>
								</div>
...
<!--									<div class="accuracy">-->
<!--										<em class="per">50</em>% 正确率-->
<!--									</div>-->
								</div>
							</div>
							<div class="work_right">
								<img src="<?= url('qrcode/zy/'.$val->id) ?>" class="qr" alt="" width="144" height="144">
								<div class="oper">
									<?php
									$nowDatetime = time();
									?>
									...
									<?php }else{ if($val->isSendMsgStudent==0){?>
                                ...
                                <?php    } }?>
									<a href="<?php echo Url::to(['/class/work-detail', 'classId'=>$classId, 'classhworkid'=>$val->id])?>" class="btn bg_white btn40 icoBtn_gou gouBtn"><i></i>去批改</a>
									<?php if($homeworkInfo->getType == 0){ ?>
									<a href="javascript:;" class="btn bg_white btn40 icoBtn_book bookBtn btn_disable"><i></i>看结果</a>
									<?php }else{?>
									<a href="<?php echo Url::to(['/workstatistical/work-statistical-all','relId'=>$val->id,'classId'=>$val->classID])?>" class="btn bg_white btn40 icoBtn_result resultBtn"><i></i>看结果</a>
									<?php } ?>
								</div>
							</div>
						</li>
