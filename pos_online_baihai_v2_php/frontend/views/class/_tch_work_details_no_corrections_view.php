...
foreach ($answerQuestionAll as $v) {
	...
	$shTestQuestion = \common\helper\QuestionInfoHelper::InfoCache($v->questionID);
	if($shTestQuestion) {
		$isMajor = $shTestQuestion->isMajorQuestionCache();
		if ($isMajor) {
			$hasMajor = true;
			break;
		}
...
<div class="stu_item">
	<div class="title item_title noBorder replyTitle">
		<div class="sUI_pannel">
			<div class="pannel_l">
				<h4><a href="#" class="blue_l_l" title="<?php echo WebDataCache::getTrueName($item->studentID); ?>">
					...
					if ($item->isCheck == 1) {
						if ($item->getType == 0) { ?>
							<a class="btn btn26 bg_blue icoBtn_add_blue"
							   href="<?= url('class/correct-pic-hom', ['classId' => $classId, 'homeworkAnswerID' => $homeworkAnswerID]) ?>">查看批改</a>
						<?php } elseif ($hasMajor == true && $item->getType == 1) { ?>
							<a class="btn btn26 bg_blue icoBtn_add_blue"
							   href="<?= url('class/correct-org-hom', ['classId' => $classId, 'homeworkAnswerID' => $homeworkAnswerID]) ?>">查看批改</a>
						<?php }
					} else { ?>
