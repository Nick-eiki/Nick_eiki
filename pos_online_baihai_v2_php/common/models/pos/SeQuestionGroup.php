...
class SeQuestionGroup extends \yii\db\ActiveRecord
{
     ...
     * @param $subjectid
     * @param $departments
     * @return array|SeQuestionGroup[]
     */
    public function defineGroup($userId,$subjectid,$departments)
    {
        $defineGroupInfo = self::find()
            ->where(['userId'=>$userId, 'subjectId'=>$subjectid, 'departmentId'=>$departments, 'groupType'=>self::DEFINE_GROUP_TYPE])
            ->select('groupId,groupName')
            ->asArray()
            ->all();
        $defineGroup = [];
        foreach($defineGroupInfo as $val){
            $questionNum = $this->defGroupQuesNum($val['groupId']);
            array_push($defineGroup,['groupId'=>$val['groupId'],'groupName'=>$val['groupName'],'QuestionNum'=>$questionNum]);
        }
        return $defineGroup;
    }
    ...
    {
        return self::find()->where(['groupId'=>$groupId])->select('groupName')->one();
    }
    ...
    {
        return self::find()->where(['groupId'=>$groupId,'userId'=>$userId])->one();
    }
    ...
    public function  deleteGroup($userId,$groupId)
    {
        ...
        try {
            ...
            foreach ($questionIds as $value) {
                $seQuestionFavoriteFolderNewModelOne =SeQuestionFavoriteFolderNew::find()->where(['questionId'=>$value->questionId,'userId'=>$userId])->one();
