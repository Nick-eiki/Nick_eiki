 ...
 * @return string
 */
function publicResources_new()
{
    return publicResources() . '/pub';
}

function publicResources_new2()
{
    return publicResources() . '/static';
}
...
/**
 * 测试代码是否执行，使用后记得去除 默认会在D盘下创建log.log文件
 * @param string|\type $data
 */
function check_run($data = "代码被执行")
{
    $path = "D:/log.log";
    ...
    if (WebDataCache::getShowTypeID($item->tqtid) == 1) {
        ...
        if (isset($item->answerContent) && !empty($item->answerContent)) {
            ...
            foreach ($imgArray as $val) {
                echo '<img width="120" height="90" style="margin-right:10px" src="' . $val . '" />';
            }
        }
...
function getNewAnswerContent($item, $level = 0)
{

    $result = '';
    if ($item == null)
        return '';
    $questionChild = $item->getQuestionChildCache();
    $tqTid = WebDataCache::getShowTypeID($item->tqtid);
    if ($tqTid == 1) {
        if (isset($item->answerContent)) {
            $result = $result . LetterHelper::getLetter($item->answerContent);
        }
    ...
    } elseif ($tqTid == 9 && empty($questionChild)) {
        $result=$item->answerContent?'对':'错';
    }
    ...
    if (empty($questionChild)) {
        ...
        foreach ($questionChild as $i) {
            $result = $result.'&nbsp;' . getNewAnswerContent($i, $level + 1);
        }
    }
    return $result;
}
...
//作业小题的选项
function getHomeworkChildQuestionOption($item , $mainId=''){
    $content = '';
    if(!empty($item->answerOption)){
        ...
        if ($item->getQuestionShowType() == 1) {
            $content = Html::radioList('answer['.$mainId.'][item][' . $item->id . ']', '', $select, array('itemOptions'=>['class'=>'radio'],'separator' => '','class'=>'radio',"encode"=>false));
        } else {
            $content = Html::checkboxList('answer['.$mainId.'][item][' . $item->id . ']', '', $select, array('itemOptions'=>['class'=>'checkbox'],'separator' => '', 'class'=>'checkbox',"encode"=>false));
        }
    }else{
        $op_list = LetterHelper::MatchOptions($item->answerOptCnt);
        if ($item->getQuestionShowType() == '1') {
            $content = '<div class="checkArea">' . Html::radioList('answer['.$mainId.'][item][' . $item->id . ']', '', ArrayHelper::map($op_list, 'id', 'content'),
                    ['itemOptions'=>['class'=>'radio'],'class' => "radio alternative", 'qid' => $item->id, 'tpid' => $item->getQuestionShowType(), 'separator' => '&nbsp;', 'encode' => false]) . '</div>';
        } elseif ($item->getQuestionShowType() == '2') {
            $content = '<div class="checkArea">' . Html::checkboxList('answer['.$mainId.'][item][' . $item->id . ']', '', ArrayHelper::map($op_list, 'id', 'content'),
                    ['itemOptions'=>['class'=>'checkbox'],'class' => "radio alternative", 'qid' => $item->id, 'tpid' => $item->getQuestionShowType(), 'separator' => '&nbsp;', 'encode' => false]) . '</div>';
        }
    }
    return $content;
}
...
//作业大题的选项
function getHomeworkMainQuestionOption($item){
    $content = '';
    if ($item->answerOption == '') {
        $op_list = LetterHelper::MatchOptions($item->answerOptCnt);
        if ($item->getQuestionShowType() == '1') {
            $content =  '<div class="checkArea">' . Html::radioList("answer[$item->id]", '', ArrayHelper::map($op_list, 'id', 'content'),
                    ['itemOptions'=>['class'=>'radio'],'class' => "radio alternative", 'qid' => $item->id, 'tpid' => $item->getQuestionShowType(), 'separator' => '&nbsp;', 'encode' => false]) . '</div>';
        } elseif ($item->getQuestionShowType() == '2') {
            $content =  '<div class="checkArea">' . Html::checkboxList("answer[$item->id]", '', ArrayHelper::map($op_list, 'id', 'content'),
                    ['itemOptions'=>['class'=>'checkbox'],'class' => "radio alternative", 'qid' => $item->id, 'tpid' => $item->getQuestionShowType(), 'separator' => '&nbsp;', 'encode' => false]) . '</div>';
        }
    } else {
        $result = json_decode($item->answerOption);
        ...
        if ($item->getQuestionShowType() == 1) {
            $content =  Html::radioList('answer[' . $item->id . ']', '', $select, array('itemOptions'=>['class'=>'radio'],'separator' => '', 'class' => 'radio', 'encode' => false));
        } else {
            $content = Html::checkboxList('answer[' . $item->id . ']', '', $select, array('itemOptions'=>['class'=>'checkbox'],'separator' => '', 'class' => 'checkbox', 'encode' => false));
        }
    }
    return $content;
}
...
function getHomeworkQuestionContent($item){
    $content = '';
    if (!empty($item->answerOption)) {
        $result = json_decode($item->answerOption);
        $result = $result == null ? array() : $result;
        $select = (from($result)->select(function ($v) {
            return  '<tr><td style="vertical-align: top;">'.LetterHelper::getLetter($v->id) . '.&nbsp;</td><td>' . $v->content.'</td></tr>';
        }, '$k')->toArray());
        $content .='<table>';
...
//作业题目（大题和小题）的选项（含radio和checkbox）2.0
function getHomeworkQuestionOption($item){
    $content = '';
    if (empty($item->answerOption)) {
        $op_list = LetterHelper::MatchOptions2($item->answerOptCnt);

        if ($item->getQuestionShowType() == 1) {
            $content =  Html::radioList("answer[$item->id]", '', ArrayHelper::map($op_list, 'id', 'content'),
                    [ 'qid' => $item->id, 'tpid' => $item->getQuestionShowType(),'encode'=>false]);
        } elseif ($item->getQuestionShowType() == 2) {
            $content =  Html::checkboxList("answer[$item->id]", '', ArrayHelper::map($op_list, 'id', 'content'),
                    [ 'qid' => $item->id, 'tpid' => $item->getQuestionShowType()]);
        }
    } else {
        $result = json_decode($item->answerOption);
        $result = $result == null ? array() : $result;
        $select = (from($result)->select(function ($v) {
            return LetterHelper::getLetter($v->id);
        }, '$k')->toArray());
        if ($item->getQuestionShowType() == 1) {
            $content =  Html::radioList('answer[' . $item->id . ']', '', $select, []);
        } else {
            $content = Html::checkboxList('answer[' . $item->id . ']', '', $select,[]);
        }
    }
    return $content;
}
...
//作业题目（大题和小题）的选项结果2.0
function getHomeworkQuestionOptionAnswer($item , $objectiveAnswer){
    $content = '';
    if ( empty($item->answerOption)) {
        $op_list = LetterHelper::MatchOptions2($item->answerOptCnt);
        foreach($op_list as $op){
            $color = '';
            $obj = $objectiveAnswer[$item->id];
