...
class AnswerController extends Controller
{
    ...
    public function actionAddSchool()
    {
        ...
        foreach ($answerList as $v) {
            $schoolID = WebDataCache::getSchoolId($v->creatorID);
            $v->schoolID = $schoolID;
            if (!$v->save()) {
                $result = false;
            }
    ...
    public function actionAddClass()
    {
        ...
        $result = true;
        foreach ($answerList as $v) {
            $userModel = SeClassMembers::find()->where(["userID" => $v->creatorID])->one();
            if ($userModel) {
    ...
    public function actionAddCountry()
    {
        ...
        $result = true;
        foreach ($answerList as $v) {
            $schoolInfo = SeSchoolInfo::find()->where(["schoolID" => $v->schoolID])->one();
            if (!empty($schoolInfo)) {
