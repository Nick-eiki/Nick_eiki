...
{
    return Yii::$app->urlManager->createUrl(yii\helpers\ArrayHelper::merge((array)$route, $params));
}
 ...
 */
function t($message, $category = 'Default', $params = array(), $source = null, $language = null)
{
...
function getanswerContent($item, $level = 0)
{
    ...
    if ($item->showTypeId == 1) {
        if (isset($item->answerContent)) {
            if ($item->answerContent === '0' || $item->answerContent === '1' || $item->answerContent === '2' || $item->answerContent === '3') {
                $result = $result . LetterHelper::getLetter($item->answerContent);
            } else {
                $result = $result . $item->answerContent;
            }

        }
    } elseif ($item->showTypeId == 2) {
        if (isset($item->answerContent)) {
    ...
    if (empty($item->childQues)) {
        return $result;
    } else {
        foreach ($item->childQues as $i) {
