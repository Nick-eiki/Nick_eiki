...
class Homeworkuse extends Model
{
    ...
    public function search($schoolID,$week_start,$week_end)
    {
         ...
         */
        $dataProvider_fun =function($schoolID, $week_start, $week_end)
        {
            //学校所有老师ID
     ...
     * 学校所有老师ID
     */
    public function teacherId($schoolID)
    {
        $teacherId = SeUserinfo::find()->where(['schoolID' => $schoolID, 'type'=>1, 'isDelete'=>0])->select('userID')->asArray()->all();
     ...
     */
    public function group($teacherId)
    {
        $query = new Query();
    ...
    * 布置作业次数-接收作业人次
    */
    public function homeworkuseSum($teacherId,$week_start,$week_end)
    {
        $query = new Query();
    ...
    * 提交作业人次
    */
    public function submitSum($teacherId,$week_start,$week_end)
    {
        $query = new Query();
