define(["popBox", 'jquery_sanhai', 'jqueryUI'], function(popBox,jquery_sanhai,jqueryUI) {
    ...
    $('#editBtn').click(function(){
        $('#editCont').removeClass().addClass('edit_input');
       ...
       html+='<tbody>';
           for(var i=0; i<data.student.length;  i++){
               ...
               for(var key in data.student[i].subject ){
                   ...
                   html+='<td><input  class="excel_text" name="'+'scoreInput['+data.student[i].autoId+']['+ key +']" title="最高分为'+data.subject[obj_l].max+'分"  data-max="'+data.subject[obj_l].max+'" type="text" value="'+data.student[i].subject[key]+'"></td>';
                   obj_l++;
               }
               html+='</tr>';
           }
