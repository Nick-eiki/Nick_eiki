define(["popBox", 'jquery_sanhai', 'jqueryUI', 'validationEngine', 'validationEngine_zh_CN'], function (popBox) {
	...
	$("#search_word").click(function () {
		...
		$.post("/personnel/teacher/index", {searchWord: searchWord,department: department, subjectId: subjectId}, function (data) {
			$("#personnel_list").html(data);
			other_opr ();
		})
	});
...
//修改用户信息
	...
	alert_ob.alert_txt_7='<div id="alert"><div id="alert_header">修改班级<div id="alert_remove"></div></div><div id="alert_main">'+
		...
		'</select>'+
		'</div>'+
		'<div style="padding:0;margin-top:48px;" class="centent"><button id="add">添加</button><br />'+
		'<button id="remove">删除</button></div>'+
		'<div class="centent">'+
		...
		'<div id="btn_c"><button id="btn_2" class="save_modifyClass">保存</button><button id="btn_3" class="cancel_modifyClass">取消</button></div></div><div id="alert_bg"></div>';
	alert_ob.alert_txt_8='<div id="alert"><div id="alert_header">移除学校<div id="alert_remove"></div></div><div id="alert_main">'+
		'<h3><span id="teacherName">11</span>&nbsp;老师是否离开本校?</h3><p style="text-align: center">离校后,该教师将与本校解除绑定关系。不能在本校平台中进行任何操作!</p></div>'+
		'<div id="btn_c"><button id="btn_2" data-userId="" class="kicked_out_school">确定</button><button id="btn_3" >取消</button></div></div><div id="alert_bg"></div>';
	...
	//根据年级获取相应的班级
	var add_class_val='';
	$("[data-btn=test]").live("mouseup",function(){
		if($("[data-btn=test] option:selected").val()!='' && add_class_val != $("[data-btn=test] option:selected").val()){
			add_class_val=$("[data-btn=test] option:selected").val();

			var gradeId = add_class_val;
	...
	$(".save_modifyClass").live('click',function(){
		if(rightBox.length == 0){
			...
			popBox.confirmBox('确认修改班级吗？', function (){
				...
				$.post('/personnel/teacher/teacher-class-modify',{userId:userId,classIdList:classIdList},function (data){
					if (data.success) {
						popBox.successBox(data.message);
						alert_ob.alert_type = true;
						$("#alert").remove();
