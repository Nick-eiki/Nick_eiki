...
//学部筛选
	...
	$(document).on("click", ".edit_info_btn", function () {
		if ($('#edit_user_info_form').validationEngine('validate')) {
			...
			//	if(pattern.test(parentsName) == false){
			//		popBox.errorBox("请正确输入家长名称！");
			//		return false;
			//	}
			//}

			//if (stuNumber.length == null) {
			//	popBox.errorBox("学号不能为空！");
			//	return false;
			//}
	...
	/*//点击弹出学生离校弹窗
	$(".addmemor_btn").live('click',function () {
		var trueName = $(this).attr("data-truename");
		var userId = $(this).attr("data-userId");
		$("#confirmBox").dialog("open");
		$("#outName").html(trueName);
		$("#confirmBox .okBtn").attr("data-userId",userId);
	});

	//对学生的离校操作
	$("#confirmBox .okBtn").live('click',function(){
		var userId = $(this).attr("data-userId");
	...
	//弹出框 学生移除本校
	alert_ob.alert_txt_1 = '<div id="alert"><div id="alert_header">离校设置<div id="alert_remove"></div></div>' +
		'<h4 style="color:#F87472;font-weight:bold;"><span id="outName"></span>&nbsp;是否离开学校？</h4>' +
		'<p style="text-align:center">离校后,该学生将于本校解除绑定关系将不能在本校中进行任何学习操作！</p>' +
		'<div id="btn_c"><button id="btn_2" data-out="leave_out">离校</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';




	//班级调动
	alert_ob.alert_txt_2 = '<div id="alert"><div id="alert_header">班级调动<div id="alert_remove"></div></div>' +
		'<h4 class="detail"><ul id="user_mes"><li>--</li><li>--</li><li>--</li><li >--</li></ul></h4>' +
		...
		//'<option value="001">1班</option>' +
		//'<option value="002">2班</option>' +
		//'<option value="003">3班</option>' +
		'</select></p>' +
		'<div id="btn_c"><button id="btn_2" data-btn="classModify">调班</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
	...
	function a(data,department,grade,class_,trueName,gradeID,className){
		$("#user_mes li").eq(0).html(trueName);
		if(department){
