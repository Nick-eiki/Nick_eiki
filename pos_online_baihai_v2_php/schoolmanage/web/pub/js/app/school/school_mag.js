define(['jquery', 'popBox','jquery_sanhai', 'validationEngine', 'validationEngine_zh_CN'], function ($, popBox) {
    ...
    //var test='';
    // 弹出框 创建班级
    alert_ob.alert_txt_1 = '<div id="alert"><div id="alert_header">创建班级<div id="alert_remove"></div>' +
    '</div><div id="alert_main"><p><span class="text_l"><i>*</i>学部:</span>' +
    ...
    '</select></span></p>' +
    '<p><span class="text_l">班级名称:</span><span class="text_r">' +
    '<input type="text" placeholder="请输入10字以内" id="class_name"></span></p>' +
    '<p><span class="text_l" style="float:left;">提示:</span>' +
    '<span class="text_r">1、"入学年份"请选择本学段中学生入学年份;<br>&nbsp;&nbsp;2、如果希望创建的班级与已存在的班级冲突,<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请确认现存班级是否继续使用,如不再使用,<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请将现有冲突班级进行<a href="javascript:;">毕业/升级/封班</a>操作。</span></p></div>' +
    '<div id="btn_c"><button id="btn_2" data-btn="add_class">保存</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //弹出框 封班
    alert_ob.alert_txt_2 = '<div id="alert" style="width:650px;"><div id="alert_header">封班<div id="alert_remove"></div></div>' +
    '<div id="alert_main"><p><span class="text_l"><i>*</i>年级:</span><span class="text_r"><select close-class="grade" id="sel_grade"><option value="0">全部</option><option value ="1">初一</option><option value ="2">初二</option><option value="3">初三</option></select></span></p>' +
    ...
    '<p class="sealed_class clearfix"><lable style="float:left;width:48px;margin-left:30px;">提示：</lable><span style="float:right;width:444px;">操作后在本班中将不能再进行任何教学活动，班级中所有成员将脱离现有班级组织关系，但仍存在于本学校中，适用于分班时操作。</spanstyle></p>'+
    '<div id="btn_c"><button id="btn_2" class="seal_class_btn">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //'<button id="seal_class_btn">封班</button></div><div id="alert_bg"></div><div id="alert_bg"></div>';
    //弹出框 升级
    alert_ob.alert_txt_3 = '<div id="alert"><div id="alert_header">升级<div id="alert_remove"></div></div>' +
    '<h4>是否确定进行操作?</h4><p style="margin-left:50px;">1、升级后,所有班级的年级属性增长一级(如一年级1班升为二年级1班),<br>毕业年级的所有班级将置为毕业状态;<br>2、本次操作不可逆;<br>3、每年限操作一次</p>' +
    '<div id="btn_c"><button id="btn_2" class="upgrade">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div><div id="alert_bg"></div>';
    //弹出框 老师移除本班
    alert_ob.alert_txt_4_1 = '<div id="alert"><div id="alert_header">移除本班<div id="alert_remove"></div></div>' +
    '<h4><span class="name_on_box">李芳</span>&nbsp;老师是否离开班级？</h4>' +
    '<p style="text-align:center">移除班级后,该老师将于本班级解除绑定关系<br>将不能在本班中进行任何教学操作！<br></p>' +
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //弹出框 学生移除本班
    alert_ob.alert_txt_4_2 = '<div id="alert"><div id="alert_header">移除本班<div id="alert_remove"></div></div>' +
    '<h4><span class="name_on_box">李芳</span>&nbsp;是否离开班级？</h4>' +
    '<p style="text-align:center">移除班级后,该学生将于本班级解除绑定关系<br>将不能在本班中进行任何学习操作！<br></p>' +
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //弹出框 学生移除本校
    alert_ob.alert_txt_4_3 = '<div id="alert"><div id="alert_header">移除学校<div id="alert_remove"></div></div>' +
    '<h4><span class="name_on_box">李芳</span>&nbsp;是否离开学校？</h4>' +
    '<p style="text-align:center">离校后,该学生将于本校解除绑定关系<br>将不能在本校中进行任何学习操作！<br></p>' +
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //弹出框修改教师身份
    alert_ob.alert_txt_5 = '<div id="alert"><div id="alert_header">修改教师身份<div id="alert_remove"></div></div>' +
    ...
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    //弹出框修改学生身份
    alert_ob.alert_txt_6 = '<div id="alert"><div id="alert_header">修改学生身份<div id="alert_remove"></div></div>' +
    '<h4><span class="name_on_box">李芳</span>&nbsp;在本班的身份将置被为</h4>' +
    '<table id="s_f"><tr>' +
    ...
    '<td><input class="radio_space" type="radio" name="s_f" value="20105">劳动委员</td>' +
    '</tr>' +
    '<tr>' +
    ...
    '</table>' +
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div><div id="alert_bg"></div>';
    alert_ob.alert_txt_7 = '<div id="alert"><div id="alert_header">添加老师<div id="alert_remove"></div></div>' +
    '<div id="alert_main"><p><span class="text_r">添加老师:</span><span class="text_l" id="glass"><input type="text" id="find_teacher" placeholder="请输入教师账号/姓名/手机号"><span></span></span></p>' +
    '<p id="no_find_name"></p>' +
    '<table id="find_name"></table>' +
    '<div id="btn_c"><button id="btn_2">确定</button><button id="btn_3">取消</button></div></div></div><div id="alert_bg"></div>';
    ...
    $("#found").live("click", function () {
        if (alert_ob.alert_type) {
            var departmentId = $(".class_grade").attr('departmentId');
            $.get("/organization/default/close-class", {departmentId:departmentId}, function (data) {
                ...
                $.each(data.classListArr,function(key,val) {
                    close_class += '<li><input type="checkbox" name="seal_class" value="'+key+'">'+val+'</li>';
                });
