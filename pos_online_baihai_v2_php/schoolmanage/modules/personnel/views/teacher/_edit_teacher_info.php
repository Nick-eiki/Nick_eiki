...
<div class="popCont">
	<div class="new_sch_con">
		<dl class="row clearfix">
			...
			<dd>
				<input type="text"
				       ...
				       name="<?php echo $teaInfo["trueName"]; ?>"
		...
		<dl class="row clearfix">
			...
			<dd>
				<label>
					<input type="radio" class="radio validate[required]" name="sex" value="0" <?php if ($teaInfo["sex"] == 0) {
						echo "checked='checked'";
					} ?>>男
				</label>
				&nbsp;&nbsp;&nbsp;
				<label>
					<input type="radio" class="radio validate[required]" name="sex" value="1" <?php if ($teaInfo["sex"] == 1) {
						echo "checked='checked'";
					} ?>>女
				</label>
			</dd>
		...
		<dl class="row clearfix">
			<dt><em>*</em>学段：</dt>
			<dd>
				<?php echo SchoolLevelModel::model()->getSchoolLevelhName($teaInfo["department"]); ?>
		...
		<dl class="row clearfix">
			...
			<dd>
				...
				&nbsp;&nbsp;
				<?php echo CHtmlExt::dropDownListAjax(Html::getAttributeName('version'), $teaInfo["textbookVersion"], EditionModel::model()->getListData(), array(
					'prompt' => '请选择版本',
					'id' => 'version',
				)) ?>
			</dd>
		...
		<dl class="row clearfix">
			<dt>任教班级：</dt>
			<dd><span><?php foreach ($classMem as $item) {
