...
<?php $form = \yii\widgets\ActiveForm::begin(array(
	...
	<div class="popCont">
		<div class="new_sch_con">
			...
			<dl class="row clearfix">
				<dt><em>*</em>手机号：</dt>
				<dd><?php echo $stuInfo["bindphone"] ?></dd>
			</dl>
			<dl class="row clearfix">
				<dt><em>*</em>性别：</dt>
				<dd>
			...
			<dl class="row clearfix">
				<dt><em>*</em>学段：</dt>
				<dd>
					<?php echo SchoolLevelModel::model()->getSchoolLevelhName($stuInfo['department']); ?>
				</dd>
			</dl>
			<dl class="row clearfix">
				<dt><em>*</em>班级：</dt>
				<dd><?php echo empty($classMembers) ? '暂无班级':\frontend\components\WebDataCache::getClassesName($stuInfo["classID"]); ?></dd>
			</dl>
			<?php if (!empty($parentsInfo)) { ?>
				...
				<dl class="row clearfix">
					<dt>家长姓名：</dt>
					<dd>
						<?php if (!empty($parentsInfo["trueName"])) {
							echo '<input type="text" class="parents_name input_txt validate[required,minSize[2]] " name="' . $parentsInfo["trueName"] . '" value="' . $parentsInfo["trueName"] . '">';
						} else {
							echo '<input type="text" class="parents_name input_txt validate[required,minSize[2]] " name="未设置" value="未设置">';
						} ?>
	...
	<div class="popBtnArea">
		<button type="button" class="okBtn edit_info_btn" uId="<?php echo $stuInfo["userID"]; ?>"
		        pId="<?php echo $parentsInfo["userID"] ?>">保存
		</button>
