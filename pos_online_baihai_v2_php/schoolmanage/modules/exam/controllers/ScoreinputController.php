...
use common\services\PublishScoreService;
...
class ScoreinputController extends SchoolManageBaseAuthController
{
    ...
    public function actionIndex()
    {
        ...
        return $this->render('index',['examClass'=>$result,'schoolExamId'=>$schoolExamId,'examName'=>$examName]);
    }
    ...
    public function actionCheckClass(){
        ...
        $schoolExamId = Yii::$app->request->getParam("examId");
        ...
        //判断当前考试下是否有这个班级
        if(!in_array($classId,$examClassId)){
            $this->notFound('不存在考试');
        }
        ...
        if($status == 0){
            return $this->render("input_select", ['examClass'=>$result,'schoolExamId'=>$schoolExamId,'classId'=>$classId,'examName'=>$examName]);
            ...
            return $this->render("input_half", ['examClass'=>$result,'schoolExamId'=>$schoolExamId,'data'=>$data,'classId'=>$classId,'examName'=>$examName]);
        }elseif($status == 2){

            //科目-满分值
            $data = $this->studentScore($examSchoolModel,$classId,$status);

            return $this->render("input_finish", ['examClass'=>$result,'schoolExamId'=>$schoolExamId,'data'=>$data,'classId'=>$classId,'examName'=>$examName]);
        }
    ...
    public function actionManualInput(){
        ...
        if($scoreInput > 0){
            ...
            //根据班级id查询本班成员
            $studentData = SeClassMembers::find()->select(['userID','memName'])->where(['classID' => $classId, 'identity' => '20403'])->andWhere(['>','userID',0])->all();
            ...
            foreach($studentData as $v){
                ...
                ExcelHelper::setSubjectScore($examScoreInput , $subjectId , 0.00);
        ...
        return $this->render("input_manual",['examClass'=>$result,'schoolExamId'=>$schoolExamId,'classId'=>$classId,'data'=>$data,'examName'=>$examName]);
    }
    ...
    public function actionAutoInput(){
       ...
       $studentScore = $this->setAutoScoreInputData($scoreInputHasUserList , $subjectData ,$classExamId);

        //未匹配的名单
        $scoreInputNoUserList = SeExamScoreInput::find()->where(['classExamId'=>$classExamId ])->andWhere(['userId'=>0])->all();
        $otherStudentScore = $this->setAutoScoreInputData($scoreInputNoUserList , $subjectData ,$classExamId);
        ...
        $data['examId'] = $schoolExamId;
        $data['classId'] = $classId;
        return $this->render('input_excel',['examClass'=>$result,'schoolExamId'=>$schoolExamId,'classId'=>$classId ,'data'=>$data ,'examName'=>$examName]);
    }

    /**
     * 生成自动录入数据格式
     */
    public function setAutoScoreInputData($scoreInputList , $subjectData ,$classExamId){
        $student = [];
        $studentScore = [];
        ...
        foreach($scoreInputList as $scoreInputHasUser){
            ...
            array_push($studentScore,$student);
        }
        return $studentScore;
    }
        ...
        if($_POST){
            ...
            try {
                ...
                foreach($scoreInput as $key=>$val){
                    ...
                    foreach($val as $k=>$v){
                        ExcelHelper::setSubjectScore($examScoreInput , $k , $v);
                        $examScoreInput->totalScore += $v;
                    }
        ...
        if($_POST){
            ...
            try {
                ...
                foreach($scoreInput as $key=>$val){
                        ...
                        foreach($val as $k=>$v){
                            ExcelHelper::setSubjectScore($examPersonalScore , $k , $v);
                            $examPersonalScore->totalScore += $v;
                        }
     ...
     * 查询班级列表
     */
    public function examClass($schoolExamId){
        //查询班级
     ...
     *
     *  查询班级学生及成绩
     * @param $seExamSchoolModel   SeExamSchool
     * @param $classId
     * @param $status
     * @return array
     */
    public function studentScore($seExamSchoolModel,$classId,$status){
       ...
       $seExamClassModel= SeExamClass::find()->where(['schoolExamId'=>$seExamSchoolModel->schoolExamId,'classId'=>$classId])->one();
        ...
        //根据班级id查询本班成员
       // $studentData = SeClassMembers::find()->select(['userID','stuID','memName'])->where(['classID' => $classId, 'identity' => '20403'])->all();
        ...
        $sysName = [];
        //echo json_encode($sysName);

        //学生成绩
        $studentScore = [];
        foreach($ScoreData as $v){

            $userName = WebDataCache::getTrueName($v->userId);
            $sysName[] = $userName;
            ...
            if($url){
                ...
                try {
                    ...
                    foreach($data as $key=>$val) {
                        ...
                        foreach($val as $k=>$v){
                            if($k != '姓名'){
                                if(!$this->judgeExcelFormat($flag,$v)){
                                    return $flag;
                                }
                            }
                        }
                        ...
                        foreach($val as $k=>$v){
                            ...
                            ExcelHelper::setSubjectScore($examScoreInput , $id , $v);
                            $examScoreInput->totalScore += $v;
     ...
     * 判断excel中数据的格式是否正确
     */
    public function judgeExcelFormat($flag,$val){
        $result = true;
        if(strlen($val) ==0){
     ...
     * @return bool
     */
    public function getScoreData( $classId , $examId){
     ...
     * 补全班级成员的成绩
     */
    public function completeScore($classId , $examId){
        ...
        //班级成员表
        $memInfo = SeClassMembers::find()->where(['classID'=>$classId,'identity'=>20403])->all();
        $memArr = [];
        ...
        if(count($scoreUserArr) < count($memArr)){
            ...
            foreach($memArr as $userId => $user){
                if(!in_array($userId , $scoreUserArr)){
     ...
     * 自动录入展示学生信息展示
     */
    public function actionGetStudentInfo($userId){
        Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
     ...
     * 发布成绩
     */
    public function actionPublishScore($examId){
     ...
     * 清空临时保存的数据
     */
    public function actionDeleteScoreInput($schoolExamId , $classId){
