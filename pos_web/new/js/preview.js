	...
	childQuesJson:[
		{
			...
			"content":'小题单选题<img src="http://avatar.csdn.net/A/2/5/1_hougelou.jpg">小题单选题<img src="http://avatar.csdn.net/A/2/5/1_hougelou.jpg">',//小题的内容
		...
		},
		{
			...
			"answerOptionJson":[
				{
					"id":"001",
					"content":'小题单选题<img src="http://avatar.csdn.net/A/2/5/1_hougelou.jpg">小题单选题<img src="http://avatar.csdn.net/A/2/5/1_hougelou.jpg">',
					"right":"1"//1正确答案，0错误答案
				},
				{
					"id":"002",
			...
			"analytical":"",//解析
			"childQuesJson":[]//小题为多选题的时候,没有小小题
		},
		{
			"quesType":"30017",//小题的类型
			"content":"填空题1111111",//小题的内容
			"answerContent":"填空题答案111111",//答案内容
			"answerOptionJson":[
			...
			"childQuesJson":[//小题的小题
				{
					"quesType":"30017",//小小题的类型
					"answerContent":"小小题1答案jjjjjjj"//答案内容
				},
				{
					"quesType":"30017",//小小题的类型
					"answerContent":"小小题2答案QQQQQQQ"//答案内容
				},
				{
					"quesType":"30017",//小小题的类型
					"answerContent":"小小题3答案KKKKKKK"//答案内容
				}
			]
		},
		{
			"quesType":"30017",//小题的类型
			"content":"填空题 22222",//小题的内容
			"answerContent":"填空题答案222222",//答案内容
			"answerOptionJson":[
...
//----------------------------------------------------------------------------	
		...
		if(sel.length>0){
			for(var i=0, len=sel.length; i<len; i++){
				...
				html+='<div class="row">';
					...
					html+='</div>';
				html+='</div>';
				html2+='<span><input  name="single" '+ (sel[i].right==1 ? 'checked' : '')+' value="'+i+'" type="'+ (showType==30001 ? "radio" : "checkbox")+'"> <label>备选项<em>'+(i+1)+'</em></label></span>';
			}
			/*html2+='<div class="errorTxt" id="errorTxt'+errorTxt_id+'" style="left:500px;top:-5px; background:red"></div?';*/
		}
		else{//没有小题数据  李泉代码
                html+='<div class="row">';
                ...
                html+='</div>';
                html2+='<span><input value="0" name="single" type="'+ (this.showType==30001 ? "radio" : "checkbox")+'" data-validation-engine= "'+(showType==1?'validate[required] radio':'validate[minCheckbox[1]] checkbox')+'" data-errormessage-value-missing="答案不能为空" > <label>备选项<em>1</em></label></span>';
	...
	onloadType:function(){
		...
		if(showType==1 || showType==2){
		   ...
		   textareas.each(function(){
			...
			if(this.childQuesJson.length>0){//如果有填空小题
				$('.ABox').hide();
				for(var i=0; i<this.childQuesJson.length; i++){
					this.addItem(this.childQuesJson[i],i)
				};
			}
			else{//没有填空小题,初始化答案富文本
				var type3_answer=UE.getEditor('answer');//初始化富文本
			};
	...
	changeType:function(sel){
		...
		if(type==30001 || type==30005){//选择题
			...
			if(this.showType!=6){//大题是6的时候,没有题目,不用渲染
	...
	addItem:function(child,i,level,j){//i小题的序列  level:第三层级(小小题) j:小题索引
		...
		//小题为单选/多选题
		if(itemType==30001||itemType==30005){
		...
		//小题为填空题
			...
			if(level==3){//是否有小小题
				...
				if(this.showType==3){//大题题型为3(填空题),并且有小题
					var html="";
					...
					/*var uq=UE.getEditor('content'+i);
                     ...
                     var ua=UE.getEditor('answerContent'+1);
						 ua.ready(function(){
						 ua.destroy();
						 UE.getEditor('answerContent'+1)
                     })*/ //保留保留
					}
			};
...
$(function(){
	...
	$('select').live('change',function(){
		...
		console.log(I_SBox_pa)
		console.log(I_ABox_pa)
	...
	//ueditor自定义新增按钮("img_vertical"图片居中对齐)
