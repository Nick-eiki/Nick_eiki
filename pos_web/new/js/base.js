...
popBox.pointTree2=function(zNodes,clickBtn,title){
	...
	//点击树上的checkbox
		function zTreeOnCheck(event, treeId, treeNode){
			...
			if(treeNode.checked==true){
				$('#treeBox .chooseLabel').show();
			$('#treeBox .labelList').append('<li val="'+treeNode.id+'"  index="'+treeNode.tId+'">'+treeNode.name+'<b class="deltreeItem">[删除]</b></li>');
				return false;
			}
			else{
				$('#treeBox .labelList li[val='+treeNode.id+']').remove();
		...
		$('.deltreeItem').live('click',function(){
			var tID= $(this).parent().attr('index');
			
			var sss=$.fn.zTree.init($("#treeBox"));
			
			var treeObj = sss.getZTreeObj(tID);
...
popBox.errorCorrect_resources=function(id){
	...
	$('#errorCorrectSendBtn').unbind('click').click(function(){
		...
		if (len.length == 0 ) {
		 ...
		 $.post('/teacher/prepare/resource-error', $("#errorCorrect_form").serialize(),
		 function (data) {
			if(data.success){
				$('.errorCorrectBox').remove();
				popBox.successBox("非常感谢您的纠错，我们会持续改进的。");
