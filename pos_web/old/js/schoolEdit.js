			...
			$('.take_addClass').live('click',function(){
					...
					/*添加班级的*/
						$('.overJs .take_ok2').bind('click',function(){
							...
							{
								$(this).parent().parent().siblings('.addL').append('<span class="add_name"><em>'+ oTxt03 +'</em><i>'+ oTxt04 + '</i></span>');		$(this).parent().hide();
				...
				html +='</div>';
					html +='</div>';
						...
						$('.overJs .take_ok').die('click').live('click',function(){
							...
							{
								$(this).parent().parent().siblings('.addL').append('<span class="add_name"><em>'+ oTxt01 +'</em><i>'+ oTxt02 + '</i></span>');		$(this).parent().hide();
								$(this).parent().siblings('.take_addClass').show();
								var oTxt01=$(this).siblings('.text_01').val('');
								var oTxt02=$(this).siblings('.text_02').val('');		
							}
